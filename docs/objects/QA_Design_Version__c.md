...
| :-------- | :---- | :--: | :---------- | 
| Activation_Date__c | Activation Date | Date | undefined |
| Deactivation_Date__c | Deactivation Date | Date | undefined |
| Department__c | Department | Text | undefined |
| isActive__c | Active | Checkbox | undefined |
| QA_Design__c | QA Design | MasterDetail | undefined |
| QA_Type__c | QA Type | Text | undefined |
| Question_Count__c | Question Count | Summary | Number of questions this design will generate |
| Total_Question_Value__c | Total Question Value | Summary | undefined |
| Version__c | Version | Number | undefined |

## Validation Rules

| Rule      | Active | Description | Formula |
| :-------- | :---- | :---------- | :------ |
| Cannot_Reactivate | Yes | Block design versions from being reactivated | isActive__c <br/>&&<br/> NOT(ISNULL(Deactivation_Date__c)) |
| Need_Department_to_Activate | Yes | Department can't be blank when activating a QA Design | ISBLANK(Department__c)<br/>&&<br/>isActive__c |
| Need_Type_to_Activate | Yes | Design needs qa type to be active | ISBLANK(QA_Type__c)<br/>&&<br/>isActive__c |
| Prevent_Date_Removal | Yes | Activation and Deactivation dates cannot be removed once entered | (ISCHANGED(Activation_Date__c)<br/>&&<br/>NOT(ISNULL(PRIORVALUE(Activation_Date__c))))<br/><br/>(ISCHANGED(Deactivation_Date__c)<br/>&&<br/>NOT(ISNULL(PRIORVALUE(Deactivation_Date__c)))) |
| Total_100_Before_Activation | Yes | The total value of the QA design must add up to 100 before activation | Total_Question_Value__c != 1<br/>&&<br/>isActive__c |
...
| 💻 | [QA_Design_Screen_Toggle_Activation](../flows/QA_Design_Screen_Toggle_Activation.md) [🕒](../flows/QA_Design_Screen_Toggle_Activation-history.md) |  Screen Flow | Activates / Deactivates current version and deactivates prior if necessary.  Updates are separated to keep validation errors relevant. |
...
| QA_Design_Version__c | [QA_Design_Before_Save_Set_Version_Number](../flows/QA_Design_Before_Save_Set_Version_Number.md) [🕒](../flows/QA_Design_Before_Save_Set_Version_Number-history.md) |  Record Before Save | Assigns version 1 for designs with no prior, and prior + 1 for all others |
...
| [QA_Design_Version_Record_Page](../pages/QA_Design_Version_Record_Page.md) |  Record Page |
