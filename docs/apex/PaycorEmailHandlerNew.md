...
```mermaid
graph TD
  PaycorEmailHandlerNew["PaycorEmailHandlerNew"]:::mainApexClass
  click PaycorEmailHandlerNew "/objects/PaycorEmailHandlerNew/"
  PaycorEmailHandlerNewTest["PaycorEmailHandlerNewTest"]:::apexTestClass
  click PaycorEmailHandlerNewTest "/apex/PaycorEmailHandlerNewTest/"
...
```java
global class PaycorEmailHandlerNew implements Messaging.InboundEmailHandler {
	...
	private static void createLeadFromPaycorEmail(Map<String, String> inboundFieldsValueMap, String htmlBodyEmail) {
		...
		// Group rpcQueue = [SELECT id, name FROM Group WHERE name = 'RPC Team' LIMIT 1];
		// Account paycorAcct = [SELECT id, name FROM Account WHERE name = 'Paycor' LIMIT 1];
        ...
        newLead.LeadSourceDetail__c = 'Payroll';
		newLead.campaign_code__c = 'Paycor Handler';
        newLead.Method_of_First_Contact__c = 'Partner Referral';
        newLead.Product_Type__c = '401(k)';
        newLead.OwnerId = '00G37000000ksAD';
        // newLead.OwnerId = rpcQueue != null ? rpcQueue.Id : null;
		// newLead.Associated_Partner__c = paycorAcct != null ? paycorAcct.id : null;
        newLead.Associated_Partner__c = '001000000033x7dAAA';
...
#### Signature
```apex
global static final VALID_STATES
```
...
### `createLeadFromPaycorEmail(inboundFieldsValueMap, htmlBodyEmail)`
...
| Name | Type | Description |
|------|------|-------------|
| inboundFieldsValueMap | Map&lt;String,String&gt; |  |
