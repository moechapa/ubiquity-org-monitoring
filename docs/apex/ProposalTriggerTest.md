...
```mermaid
graph TD
  ProposalTriggerTest["ProposalTriggerTest"]:::mainApexClass
  click ProposalTriggerTest "/objects/ProposalTriggerTest/"
...
public class ProposalTriggerTest {
	...
	static void makeData() {
		...
		try{
			...
			version.New_Version__c = false;
			update version;
		}  catch (QueryException e) {
            version = new Simply_Version__c(New_Version__c = false);
            insert version;
		}
		Account acc = DataFactory.createAccount();
		Opportunity opp = DataFactory.createOpportunity(acc);
		List<Proposal__c> propLst = DataFactory.createProposals(1, opp);
